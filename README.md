...
3. **_iOS_ CSS `transition-delay` 偶现闪烁**
...
4. **声明 `virtualHost: true` 时自定义组件实例不具备被选择以及被观测的特性**
...
5. **声明 `styleIsolation: shared` 时 `externalClasses` 属性无效**
...
6. **`<image/>` 开启 `widthFix` 或 `heightFix` 模式时，尺寸样式自动计算的响应速度差强人意**
...
7. **_iOS_ `font-family` 覆盖字符范围不全导致中英文混合排版时字重异常**
...
8. **应用自定义英文字体样式的段落的 line box 高度异常**
